  ...
  group "boundary-controller" {
    count = 1
     network {
      ...
      # }
      artifact {
        source     = "https://releases.hashicorp.com/boundary/0.2.2/boundary_0.2.2_linux_amd64.zip"
        destination = "tmp/"
        options {
          checksum = "sha256:599ef6b12a2f9c8f3eba79b4c91066f7ca0a4303381cffedb0bfb1843e546361"
        }
      }
      template {
        data        = <<TEMPLATEEOF
...
tmp/boundary database init \
  -format json \
  -config=tmp/config.hcl > init.txt

echo "--> init output"
cat init.txt

# echo "-->  checking to see if database already initialized before writting to consul"
if grep -Fxq "Database has already been initialized" init.txt
then
  echo "--> Database already initialized, skipping"
else
  echo "--> adding to consul"
  consul kv put service/boundary/boundary-init @init.txt
fi
